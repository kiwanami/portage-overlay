...
# $Header: /var/cvsroot/gentoo-x86/www-servers/nginx/nginxs3-1.4.1-r5.ebuild,v 1.2 2013/06/02 22:48:30 naota Exp $
...
# S3
AWSAUTH_MODULE_PV="1.0.0"
AWSAUTH_MODULE_NGINX_PV="1.4.1"
AWSAUTH_MODULE_P="ngx_aws_auth-${AWSAUTH_MODULE_PV}"
AWSAUTH_MODULE_URI="https://github.com/anomalizer/ngx_aws_auth/archive/${AWSAUTH_MODULE_PV}.tar.gz"
AWSAUTH_MODULE_WD="${WORKDIR}/ngx_aws_auth-${AWSAUTH_MODULE_PV}"
...
SRC_URI="http://nginx.org/download/${P}.tar.gz
	aws_s3? ( ${AWSAUTH_MODULE_URI} -> ${AWSAUTH_MODULE_P}.tar.gz )
...
aws_s3 syslog userland_GNU vim-syntax"
...
src_configure() {
	...
	# s3 support
	if use aws_s3; then
		myconf+=" --add-module=${AWSAUTH_MODULE_WD}"
	fi
...
src_install() {
	...
	if use aws_s3; then
		docinto ${AWSAUTH_MODULE_P}
		dodoc "${AWSAUTH_MODULE_WD}"/README
	fi
